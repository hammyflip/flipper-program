...
then
  # Deploy program.
  echo -e "Calling solana program deploy target/deploy/flipper_program.so -u $ENVIRONMENT -k ./keys/$ENVIRONMENT/deployer-keypair.json --program-id ./keys/$ENVIRONMENT/program-keypair.json\n"
